...
pub fn flatten_pa<S: Scalar, F: FnMut(&LineSegment<S>, Range<S>)>(
    ...
    for t_inflection in inflections {
        ...
        if let Some(tf) = inflection_approximation_range(&after, tolerance) {
            ...
            t += tf * (S::ONE - t);
            call_back(&LineSegment { from, to: after.from}, t0..t);
        }
...
fn flatten_cubic_no_inflection_with_t<S: Scalar, F: FnMut(&LineSegment<S>, Range<S>)>(
    mut curve: CubicBezierSegment<S>,
    mut t0: S,
    t1: S,
    tolerance: S,
    ...
    loop {
        ...
        if step >= S::ONE {
            if t0 < t1 {
                call_back(&LineSegment { from, to: end }, prev..t1);
            }
            break;
        }
        ...
        curve = curve.after_split(step);
        t0 += step * (t1 - t0);
        call_back(&LineSegment { from, to: curve.from}, prev..t0);
    }
}
...
fn test_iterator_builder_1() {
    ...
    flatten_pa(&c1, tolerance, &mut |seg, _| {
        builder_points.push(seg.to);
    });
...
fn test_issue_194() {
    ...
    flatten_pa(&segment, 0.1, &mut |p, _| {
        points.push(p.to);
    });
...
fn flatten_with_t() {
    ...
    for tolerance in &[0.1, 0.01, 0.001, 0.0001] {
        ...
        let mut a = Vec::new();
        flatten_pa(&segment, tolerance, &mut |p, _| {
            a.push(p.to);
        });

        let mut b = Vec::new();
        let mut ts = Vec::new();
        flatten_pa(&segment, tolerance, &mut |p, t| {
            b.push(p.to);
            ts.push(t.end);
        });
