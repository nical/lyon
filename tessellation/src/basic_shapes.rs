...
use crate::{
    FillOptions, FillTessellator, Side, StrokeAttributes, StrokeAttributesData, StrokeOptions,
    TessellationResult, VertexId, VertexSource,
};
...
pub fn fill_quad(
    ...
    output: &mut dyn BasicGeometryBuilder,
) -> TessellationResult {
...
pub fn fill_rectangle(
    ...
    let a = output.add_vertex(rect.origin)?;
    let b = output.add_vertex(bottom_left(&rect))?;
    let c = output.add_vertex(bottom_right(&rect))?;
    let d = output.add_vertex(top_right(&rect))?;
    output.add_triangle(a, b, c);
    output.add_triangle(a, c, d);
...
fn stroke_thin_rectangle(
    ...
    let b = output.add_stroke_vertex(bottom_left(&rect), StrokeAttributes(&mut attributes))?;
    ...
    let c = output.add_stroke_vertex(bottom_right(&rect), StrokeAttributes(&mut attributes))?;

    attributes.normal = vector(1.0, -1.0);
    attributes.advancement += rect.size.height;

    let d = output.add_stroke_vertex(top_right(&rect), StrokeAttributes(&mut attributes))?;
...
pub fn fill_rounded_rectangle(
    ...
    let v = [
        output.add_vertex(p7)?,
        output.add_vertex(p6)?,
        output.add_vertex(p5)?,
        output.add_vertex(p4)?,
        output.add_vertex(p3)?,
        output.add_vertex(p2)?,
        output.add_vertex(p1)?,
        output.add_vertex(p0)?,
    ];
...
fn fill_border_radius(
    ...
    let vertex = output.add_vertex(position)?;
...
pub fn fill_circle(
    ...
    let v = [
        output.add_vertex(center + (left * radius))?,
        output.add_vertex(center + (up * radius))?,
        output.add_vertex(center + (right * radius))?,
        output.add_vertex(center + (down * radius))?,
    ];
