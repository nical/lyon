use crate::allocator::*;
use crate::{TransferBufferWriter, GpuBlock, GPU_BLOCK_SIZE, GpuData, BufferId};
...
pub fn as_mut_bytes<T: GpuData>(slice: &mut [T]) -> &mut[u8] {
    unsafe {
...
pub fn as_bytes<T: GpuData>(slice: &[T]) -> &[u8] {
    unsafe {
...
impl<'l> WritableMemory<'l> {
    ...
    pub fn new_transfer_buffer_writer(&self, buffer_id: BufferId) -> TransferBufferWriter {
        TransferBufferWriter::new(self.new_writer(), buffer_id)
    }
