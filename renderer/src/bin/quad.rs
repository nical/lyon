...
use renderer::writer::WritableMemory;
use renderer::writer::*;
use renderer::gfx;

use gfx::{
    traits::*,
    buffer, command, image, memory, pass, pool, pso, format,
...
fn main() {
    ...
    let vbo_id: GpuBuffer<GpuBlock> = GpuBuffer::new(0);

    {
        ...
        ).unwrap();
        {
            ...
            let address = writer.write_front(&QUAD);
            assert_eq!(address.unwrap().range.start, 0);
        }
    ...
    let pipeline = {
        let vs_module = {
            let spirv = fs::read(concat!(env!("OUT_DIR"), "/shader.vert.spirv")).unwrap();
            device.create_shader_module(&spirv).unwrap()
        };
        let fs_module = {
            let spirv = fs::read(concat!(env!("OUT_DIR"), "/shader.frag.spirv")).unwrap();
            device.create_shader_module(&spirv).unwrap()
        };
...
fn make_image(w: u32, h: u32) -> Vec<u8> {
    ...
    for y in 0..h {
        for x in 0..w {
            let r = (x * 255 / w) as u8;
            let g = (y * 255 / h) as u8;
            let b = if (x % 100 < 50) ^ (y % 100 < 50) { 0 } else { 255 } as u8;
            let a = 255;
            buf.push(r);
            buf.push(g);
            buf.push(b);
