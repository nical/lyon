    ...
    fn render_command(&mut self, command: &gpu::RenderingCommand) {
        match *command {
            ...
            gpu::OpDraw(ref draw) => {
                ...
                gl::BindBuffer(gl::ARRAY_BUFFER, draw.geometry.vertices.handle);

                let mut i : u32 = 0;
                for attribute in draw.geometry.layout.iter() {
                    unsafe {
                        // TODO[nical] disable attribs when not used!
                        gl::EnableVertexAttribArray(i);
                        gl::VertexAttribPointer(i,
                                                attribute.components,
                                                gl_attribue_type(attribute.attrib_type),
                                                gl::FALSE, // TODO: normalize
                                                attribute.stride,
                                                attribute.offset as *c_void);
                    }
                    i += 1;
                }

                if draw.geometry.elements.handle != 0 {
                    unsafe {
                    gl::DrawElements(gl_draw_mode(draw.mode),
                                     draw.geometry.count, gl::UNSIGNED_INT,
                                     cast::transmute(0));
                    }
                } else {
...
impl gpu::RenderingContext for RenderingContextGL {
    ...
    fn link_shader_program(&mut self, p: gpu::ShaderProgram, shaders: &[gpu::Shader]) -> Result<gpu::ShaderProgram, ~str> {
        unsafe {
    ...
    fn get_shader_input_location(&mut self, program: gpu::ShaderProgram, name: &str) -> i32 {
