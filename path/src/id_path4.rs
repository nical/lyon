            ...
            match self.cmds.next() {
                ...
                Some(&PathOp { verb: Verb::Cubic }) => {
                    ...
                    Some(PathEvent::Cubic {
                        from: &self.points[from],
                        ctrl1: &self.points[ctrl1],
                        ctrl2: &self.points[ctrl2],
                        to: &self.points[to],
                    })
                }
                Some(&PathOp { verb: Verb::End }) => {
...
impl IdPathBuilder {
    ...
    pub fn with_capacity(
        ...
        Self {
            points: Vec::with_capacity(n_endpoints + n_ctrl_points),
    ...
    fn add_ctrl_point(&mut self, cp: Point) -> CtrlPointId {
        let id = CtrlPointId(self.points.len() as u32);
        self.points.push(cp);
        id
    }
}
